"""CLI to subdivide V2 townlayout blocks into parcels and alleys.

Purpose
-------
Rebuild through Gate A, protect open spaces, load the stamp index, and
bisect wards into parcels. Writes ``parcels.json`` and a diagnostic PNG.

Pipeline position
-----------------
V2 townlayout Phase 16; no stamp seating/VTEX.
"""
...
from procgen.townlayout.blocks import inset_blocks  # noqa: E402
...
from procgen.townlayout.openspaces import finalize_open_spaces  # noqa: E402
from procgen.townlayout.parcels import (  # noqa: E402
    subdivide_parcels,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build V2 parcels (Phase 16)")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        blocked = inset_blocks(warded)
        opened = finalize_open_spaces(blocked, brief, candidate_id=cid)
        index = build_stamp_index(kit, load_stamp_libraries(DEFAULT_LIBRARIES))
        product = subdivide_parcels(
            opened, brief, index,
            p50=float(ctx.stamp_footprint_stats["p50"]),
            candidate_id=cid, master_seed=int(brief["master_seed"]))
