"""CLI to place V2 market/keep anchors on the inner city.

Purpose
-------
Rebuild through Phase 6, reserve the market plaza patch, write
``anchors.json`` and ``anchors_diagnostic.png``.
...
from procgen.townlayout.anchors import place_anchors, write_anchors_diagnostic  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Place V2 townlayout anchors")
...
def main(argv=None) -> int:
    ...
    if topdown is None:
        print("anchors_diagnostic.png skipped: missing site_topdown.png",
              file=sys.stderr)
    else:
        survey = json.loads(Path(args.survey).read_text(encoding="utf-8"))
        write_anchors_diagnostic(
            ctx, product, topdown_path=topdown, survey=survey,
            out_png=out_dir / "anchors_diagnostic.png")
    return 0
